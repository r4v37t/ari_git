<div class="row-fluid">
    <div class="span12">
        ...
        if (isset($_POST['tambah'])) {
			...
			<?php } else {
            $q = mysql_query("insert into login values('$id','$sandi','$nama',30,$ref)");
            if ($q) {
                $q = mysql_query("insert into blog_konten values('sambutan:$ref','ini sambutan')");
                $q = mysql_query("insert into blog_konten values('struktur:$ref','ini struktur')");
                $q = mysql_query("insert into blog_konten values('visimisi:$ref','ini visimisi')");
                ?>
                    ...
                    <form method="post" class="form-horizontal">
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <select name="ref">
                                    <?php
                                    $qq = mysql_query("select * from sia_ekstra");
                                    while ($hh = mysql_fetch_array($qq)) {
                                        ?>
                                        <option value="<?php echo $hh['ekstra_id']; ?>" <?php echo ($h['ref'] == $hh['ekstra_id']) ? 'selected' : ''; ?> ><?php echo "$hh[nama]"; ?></option>
        <?php
    }
    ?>
                                </select>
                        ...
                        <div class="form-actions">
                            ...
                            <button type="button" class="btn btn-warning" onclick="location.href = '?menu=blog'">Batal</button>
                        </div>
                    </form>
                </div>
            </div>
                            ...
                            <div class="widget-content nopadding">
                                <form method="post" class="form-horizontal">
								    <div class="control-group">
                                        <label class="control-label">Nama Ekstrakulikuler :</label>
                                        <div class="controls">
                                            <select onchange="datablog(this.value)" name="ref">
                                                <?php
                                                $q = mysql_query("select * from sia_ekstra");
                                                while ($h = mysql_fetch_array($q)) {
                                                    ?>
                                                    <option value="<?php echo $h['ekstra_id']; ?>"><?php echo "$h[nama]"; ?></option>
												<?php
												}
											?>
                                            </select>
            ...
            <div class="widget-box">
                ...
                <div class="widget-content nopadding">
                    <table class="table table-bordered data-table">
                        ...
                        <tbody>
                            <?php
                            $q = mysql_query("select * from login where level=30");
                            while ($h = mysql_fetch_array($q)) {
                                $qq = mysql_query("select * from sia_ekstra where ekstra_id=$h[ref]");
                                ...
                                <tr>
                                    ...
                                    <td><center><a href="?menu=blog&edit=<?php echo $h['username']; ?>">Edit</a> | <a href="?menu=blog&del=<?php echo $h['username']; ?>">Del</a></center></td>
                            </tr>
        <?php
    }
    ?>
...
function datablog(str) {
    if (str.length == 0) {
        ...
        xmlhttp.open("GET", "datablog.php?q=" + str, true);
        xmlhttp.send();
    }
}
</script>