<div class="row-fluid">
	<div class="span12">
		...
		if(isset($_POST['tambah'])){
			...
			$q=mysql_query("insert into login values('$id','$sandi','$nama',20,0)");
			if($q){
			?>
		...
		if(isset($_GET['del'])){
			...
			?><script>setTimeout(function(){ location.href='?menu=sia'; },200);</script><?php
		}
			...
			<div class="widget-content nopadding">
				<form method="post" class="form-horizontal">
					...
					<div class="form-actions">
						...
						<button type="button" class="btn btn-warning" onclick="location.href='?menu=sia'">Batal</button>
					</div>
				</form>
			</div>
		</div>
		...
		<div class="widget-box">
			...
			<div class="widget-content nopadding">
				<table class="table table-bordered data-table">
					...
					<tbody>
						<?php
						$q=mysql_query("select * from login where level=20");
						while($h=mysql_fetch_array($q)){
						?>
						<tr>
							<td><?php echo $h['nama']; ?></td>
							<td><?php echo $h['username']; ?></td>
							<td><center><a href="?menu=sia&edit=<?php echo $h['username']; ?>">Edit</a> | <a href="?menu=sia&del=<?php echo $h['username']; ?>">Del</a></center></td>
						</tr>
						<?php
						}
						?>
