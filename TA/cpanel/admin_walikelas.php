<div class="row-fluid">
	<div class="span12">
		...
		if(isset($_POST['tambah'])){
			...
			<?php } else {
			$q=mysql_query("insert into login values('$id','$sandi','$nama',40,$ref)");
			if($q){
			?>
		...
		if(isset($_GET['del'])){
			...
			?><script>setTimeout(function(){ location.href='?menu=walikelas'; },200);</script><?php
		}
			...
			<div class="widget-content nopadding">
				<form method="post" class="form-horizontal">
					...
					<div class="control-group">
						...
						<div class="controls">
							<select name="ref">
								...
								?>
								<option value="<?php echo $hh['kelas_id']; ?>" <?php echo ($h['ref']==$hh['kelas_id'])?'selected':''; ?> ><?php echo "$hh[nama]"; ?></option>
								<?php
								}
								?>
							</select>
					...
					<div class="form-actions">
						...
						<button type="button" class="btn btn-warning" onclick="location.href='?menu=walikelas'">Batal</button>
					</div>
				</form>
			</div>
		</div>
						...
						<div class="widget-content nopadding">
							<form method="post" class="form-horizontal">
								<div class="control-group">
									<label class="control-label">Nama Kelas :</label>
									<div class="controls">
										<select onchange="ambildata(this.value)" name="ref">
											<?php
		...
		<div class="widget-box">
			...
			<div class="widget-content nopadding">
				<table class="table table-bordered data-table">
					...
					<tbody>
						<?php
						$q=mysql_query("select * from login where level=40");
						while($h=mysql_fetch_array($q)){
							$qq=mysql_query("select * from sia_kelas where kelas_id=$h[ref]");
						...
						<tr>
							...
							<td><center><a href="?menu=walikelas&edit=<?php echo $h['username']; ?>">Edit</a> | <a href="?menu=walikelas&del=<?php echo $h['username']; ?>">Del</a></center></td>
						</tr>
						<?php
						}
						?>
...
function ambildata(str) {
    if (str.length == 0) {
        ...
        xmlhttp.open("GET", "ambildata.php?q=" + str, true);
        xmlhttp.send();
    }
}
</script>